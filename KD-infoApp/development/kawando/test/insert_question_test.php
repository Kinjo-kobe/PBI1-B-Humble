...
<body>
    ...
    // 仮データを挿入する
    try {
        $sql = 'insert into questions (user_id, question_title, question_text) values (:userId, :questionTitle, :questionText)'; // SQL文を変数に代入
        $stmt = $pdo->prepare($sql); // SQL文実行する前にprepareメソッドを実行
        $stmt->execute(['userId' => $userId, 'questionTitle' => $questionTitle, 'questionText' => $questionText]); // 値を代入してSQL文を実行
